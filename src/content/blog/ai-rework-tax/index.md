...
![Datadog monitor status page showing 21 alerts across multiple services with metric graphs. Source: Datadog docs.](./datadog-alert.webp)
...
![Pass rate comparison: alert only 51%, alert plus captures 77%, plus 26 percentage points](./pass-rate-comparison.svg)
...
![How the 100 bugs split: 40 both passed, 37 captures rescued, 11 captures hurt, 12 both failed](./divergence-breakdown.svg)
...
![Wrong service rate: 34% with alert only, 4% with captures](./wrong-service-rate.svg)
...
![The confidence trap: without captures the agent was slow and exploratory but correct, with captures it was fast and decisive but wrong](./confidence-trap.svg)
